...
{	
	...
	/*==== title bar ====*/				
		...
		titleBar.rows[0].insertCell(-1);
		titleBar.rows[0].cells[0].innerHTML = '<b>Name Villages</b> (max 32 characters) - Fixed by LilGhost';
	...
	if(nameSaveButtons.length > 0){
		window.setTimeout(
			function(){
				...
				if(nameSaveButtons.length > 0){
					UI.InfoMessage( ('Saved village name #'+savedCounter),5000,'success');
					saveVillageNames();
				}else{
					UI.InfoMessage('All village names have been saved.',5000,'success');
				}
			},
			saveDelay
		);
