...
var scriptData = {
	...
	helpLink: 'https://forum.tribalwars.net/index.php?threads/extended-player-info.285361/',
};
...
var translations = {
	en_DK: {
		...
		Help: 'Help',
		'Script must be executed from Player Info screen!': 'Script must be executed from Player Info screen!',
	...
	sk_SK: {
		...
		Help: 'Pomoc',
		'Script must be executed from Player Info screen!': 'Skript musí byť spustený z profilu hráča!',
	...
	pt_PT: {
		...
		Help: 'Ajuda',
		'Script must be executed from Player Info screen!': 'O script deve ser executado no perfil de um jogador!',
		'Error while fetching "village.txt"!': 'Error while fetching "village.txt"!',
		'Points:': 'Pontos:',
		'Rank:': 'Rank:',
		'Player:': 'Jogador:',
		...
		'Villages:': 'Aldeias:',
		'Toggle Graphs': 'Mostrar gráficos',
		'Toggle Coords': 'Mostrar coordenadas',
		'Toggle Random Fake Scripts': 'Mostrar Fake Scripts Aleatórios',
		'Toggle Sequential Fake Scripts': 'Mostrar Fake Scripts Sequenciais',
		'TWStats Player Profile': 'Perfil do jogador no TWStats',
		'Show Player on Global Map': 'Mostrar jogador no mapa mundo',
		'TribalWars Maps Player Profile': 'Perfil do jogador no  TribalWars Maps',
		'Village Coords for Continent': 'Coordenadas das aldeias no Continente',
		'village/s': 'aldeia/s',
		'Script is already loaded!': 'O script já foi executado e já está a funcionar!',
	},
	pt_BR: {
		'Extended Player Info': 'Mais informação do jogador',
		Help: 'Ajuda',
...
function initExtendedPlayerInfo(playerId) {
	...
	var filteredVillagesByContinent = getFilteredVillagesByContinent(playerVillageCoords, continents);
	...
	if (DEBUG) {
		...
		console.debug(`${scriptInfo()} Villages Coords String:`, playerVillageCoordsString);
		console.debug(`${scriptInfo()} Continents:`, continents);
		console.debug(`${scriptInfo()} Villages by Continent:`, filteredVillagesByContinent);
	}
	...
	if (DEBUG) {
		console.debug(`${scriptInfo()} TWStats Player Profile:`, twStatusProfile);
	...
	var playerPoints = jQuery('#player_info tbody tr:eq(2) td:eq(1)').text().trim();
	var playerRank = jQuery('#player_info tbody tr:eq(3) td:eq(1)').text().trim();
...
function getFilteredVillagesByContinent(playerVillagesCoords, continents) {
	...
	var result = groupArrayByProperty(filteredVillagesByContinent, 'continent', 'coords');
...
function buildMapPlayerUrl(playerId) {
	return `//${game_data.market === 'en' ? '' : game_data.market + '.'}twstats.com/${
		game_data.world
...
(function () {
	...
	if (gameScreen === 'info_player') {
		...
		UI.ErrorMessage(`${tt('Script must be executed from Player Info screen!')}`, 4000);
	}
})();
