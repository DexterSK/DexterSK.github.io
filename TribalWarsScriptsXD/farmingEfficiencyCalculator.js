...
var scriptData = {
	...
	helpLink: 'https://forum.tribalwars.net/index.php?threads/farming-efficiency-calculator.285288/',
};
...
var translations = {
	...
	it_IT: {
		...
		'Total Estimated:': 'Totale Saccheggiabile:',
...
// Constants
const API_ENDPOINT = '/game.php?screen=report&ajax=view&id=';
...
function init() {
	...
	if (isReportsOverviewSCreen) {
		...
		UI.ErrorMessage(
			`${tt('Script must be run from')} <a href="/game.php?screen=report&mode=attack" class="btn">${tt(
				'Reports Overview'
			)}</a>`,
			4000
		);
...
function getFarmingData() {
	...
	reportIds.asyncForEach(async function (reportId) {
		await wait(() => {
			var fetchReportUrl = API_ENDPOINT + reportId;
...
function doFarmingCalculations() {
	...
	farmingEfficiency = ((totalLootedResources / totalEstimatedResources) * 100).toFixed(2);

	var lootedPerVillage = (totalLootedResources / resourcesLootedArray.length).toFixed(0);
...
function parseReport(reportData) {
	var reportHtmlString = reportData.dialog;
	...
	if (attackResults !== null) {
		...
		var hauledResources = attackRes.children('tbody').children('tr:first-child').children('td:last-child').text();

		var hauledWood = $(attackRes).find('tr:first-child .nowrap:eq(0)').text().replace('.', '');
		var hauledClay = $(attackRes).find('tr:first-child .nowrap:eq(1)').text().replace('.', '');
		var hauledIron = $(attackRes).find('tr:first-child .nowrap:eq(2)').text().replace('.', '');
