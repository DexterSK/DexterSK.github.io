...
var scriptConfig = {
    scriptData: {
        ...
        version: 'v1.3.4',
        author: 'RedAlert',
        authorUrl: 'https://dextersk.github.io/TribalWarsScripts/',
        helpLink:
            'https://dextersk.github.io/TribalWarsScripts/',
    },
    translations: {
        en_DK: {
            "Scripts Pack": "RedAlert's Scripts Pack",
            Help: 'Help',
    ...
    allowedMarkets: ['en', 'us', 'pt', 'fr', 'br', 'de', 'sk'],
    allowedScreens: [],
    allowedModes: [],
    isDebug: DEBUG,
    enableCountApi: true,
...
$.getScript('https://dextersk.github.io/TribalWarsScripts/Dominecka/twSDK.js', function () {
    // Initialize Library
    ...
    function handleAddToQuickBar() {
        jQuery('.add-to-quick-bar').on('click', function (e) {
            ...
            let selectedScript = jQuery(this).parent().parent().find('.ra-textarea').text().trim();
            let selectedScriptName = jQuery(this).parent().parent().find('.ra-script-title').attr('data-camelize').trim();

            let scriptData = `hotkey=&name=${selectedScriptName}&href=${encodeURI(selectedScript)}`;
            let action = "/game.php?screen=settings&mode=quickbar_edit&action=quickbar_edit&";

            jQuery.ajax({ url: action, type: "POST", data: scriptData + `&h=${csrf_token}` });
    ...
    function prepareContent(scripts) {
        ...
        let content = `
			<div class="ra-mb15">
				<input type="search" placeholder="${twSDK.tt('Search scripts ...')}" id="searchScripts" class="ra-input">
			</div>
			<div class="ra-mb15">
				<p>
					<b><span id="raScriptsCount">${scripts.length}</span> ${twSDK.tt('scripts listed')}</b>
				</p>
			</div>
			<div class="ra-mb15">
				${scriptCategories}
        ...
        scripts.forEach((script, index) => {
            ...
            content += `
				<tr class="script-${id}">
					...
					<td class="ra-tal">
						<a href="${url}" target="_blank" rel="noreferrer noopener" data-camelize="${camelize(cleanTitle)}" title="${cleanTitle}" class="ra-script-title">
					...
					<td>
						<a href="${help_link}" target="_blank" rel="noreferrer noopener" class="ra-external-icon-link" title="${twSDK.tt('Go to forum')}">
							📜
						</a>
					</td>
					<td>
						<a href="${demo_video}" target="_blank" rel="noreferrer noopener" class="ra-external-icon-link ra-view-video" title="${twSDK.tt('View demo video')}">
							🎬
						</a>
					</td>
                    <td>
                        <a href="javascript:void(0);" class="add-to-quick-bar" title="${twSDK.tt('Add script on Quick-bar')}">
                            ➕
                        </a>
                    </td>
				</tr>
    ...
    async function fetchScripts() {
        try {
            ...
            const response = await jQuery.get('https://twscripts.dev/api/fetch-scripts/');
